        ...
        <form method="POST" action="{{ route('admin.pages.store') }}">
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="slug" value="{{ $old['slug'] ?? '' }}" class="form-control" placeholder="auto">
                </div>
            </div>
            <div class="mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" class="form-select" required>
                        @foreach (['draft', 'published'] as $st)
                            <option value="{{ $st }}" {{ ($old['status'] ?? 'published') === $st ? 'selected' : '' }}>{{ ucfirst($st) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
            ...
            <details class="mb-3">
                ...
                <div class="row g-3">
                    <div class="col-md-6"><input type="text" name="meta_title" value="{{ $old['meta_title'] ?? '' }}" class="form-control" placeholder="Meta Title"></div>
                    <div class="col-md-6"><input type="text" name="meta_keywords" value="{{ $old['meta_keywords'] ?? '' }}" class="form-control" placeholder="Meta Keywords"></div>
                    <div class="col-12"><textarea name="meta_description" rows="2" class="form-control" placeholder="Meta Description">{{ $old['meta_description'] ?? '' }}</textarea></div>
                </div>
            </details>
            ...
            <div class="d-flex gap-2">
                <button class="btn btn-primary"><i class="bi bi-check-circle me-1"></i> Create Page</button>
