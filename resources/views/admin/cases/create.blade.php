        ...
        <form method="POST" action="{{ route('admin.cases.store') }}">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="case_number" value="{{ $old['case_number'] ?? '' }}" class="form-control" required>
                </div>
                <div class="col-md-8">
                ...
                <div class="col-md-6">
                    ...
                    <select name="practice_area_id" class="form-select">
                        ...
                        @foreach ($practiceAreas as $area)
                            <option value="{{ $area->id }}" {{ ($old['practice_area_id'] ?? '') == $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="court_case_number" value="{{ $old['court_case_number'] ?? '' }}" class="form-control">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="fees" step="0.01" min="0" value="{{ $old['fees'] ?? '' }}" class="form-control">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Filed Date</label>
                    <input type="date" name="filed_date" value="{{ $old['filed_date'] ?? '' }}" class="form-control">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Next Hearing Date</label>
                    <input type="date" name="next_hearing_date" value="{{ $old['next_hearing_date'] ?? '' }}" class="form-control">
                </div>
                <div class="col-12">
                ...
                <div class="col-12 d-flex gap-2 pt-2">
                    <button class="btn btn-primary"><i class="bi bi-check-circle me-1"></i> Create Case</button>
                    <a href="{{ route('admin.cases.index') }}" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </div>
        </form>
    </div>
