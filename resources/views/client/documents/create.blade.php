    ...
    <div class="client-card rounded-xl p-6 max-w-2xl">
        ...
        <form method="POST" action="{{ route('client.documents.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <small class="text-muted">Allowed: PDF, DOC, DOCX, JPG, PNG, GIF, TXT (max 10MB each)</small>
            </div>
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="document_type" class="form-select" required>
                        @foreach (['general', 'agreement', 'evidence', 'id_proof', 'other'] as $type)
            ...
            <div class="form-check form-switch mb-3">
                <input class="form-check-input" type="checkbox" name="is_shared" value="1" id="is_shared" {{ old('is_shared') ? 'checked' : '' }}>
                <label class="form-check-label" for="is_shared">Share with my advocate</label>
            </div>
            <div class="d-flex gap-2">
                <button class="btn btn-primary"><i class="bi bi-upload me-1"></i> Upload</button>
                <a href="{{ route('client.documents.index') }}" class="btn btn-outline-secondary">Cancel</a>
            </div>
        </form>
    </div>
@endsection